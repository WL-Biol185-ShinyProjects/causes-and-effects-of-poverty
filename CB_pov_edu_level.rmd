...
```{r}
Edu_Levels <- c("All Education Levels", "No High School Degree","Some University","University (4 Years) Or Higher")
Percent_Below_Poverty1 <- c(9.5, 25.1,13.1,8.5)
Graph_Edu <- data.frame(Edu_Levels, Percent_Below_Poverty1)
ggplot(data=Graph_Edu,aes (x = Edu_Levels, y = Percent_Below_Poverty1, fill=Edu_Levels)) + geom_bar(stat="identity") + theme_classic() + theme(axis.text.x = element_text(angle=50, hjust =1)) + labs(title="Percentage of People Below Poverty Threshold by Education Level",
